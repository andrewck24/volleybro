...
| 0.5 | modal / floating surfaces | `--popover` | ~10% (change) | ~97% (change) |
| 1 | card / item | `--card` | 14.5% (keep) | 98.45% (keep) |

- Body switches from `bg-accent` to `bg-background`.
- `--popover` is repurposed as the 0.5 layer; Dialog uses `bg-popover`, and Popover/Select/Dropdown content share this layer (their `shadow-md` keeps them visibly raised).

...
- [ ] 4.1 Create `.github/workflows/changesets.yml` with the following structure (decision: Replace `release.yml` with `changesets.yml`; decision: `changesets/action@v1` without `publish` input):
...
- [ ] 4.2 [P] Update `.github/workflows/chromatic.yml`:
...
- [ ] 4.3 Commit: `chore(ci): replace release workflow with changesets, update chromatic to pnpm`

## 5. Documentation and Config Sync

- [ ] 5.1 [P] Update `openspec/config.yaml`: change `Package manager: npm` (line 7) → `Package manager: pnpm`, and change `npm run test, npm run lint, npx tsc -noEmit, npm run build` (line 61) → `pnpm test, pnpm lint, pnpm exec tsc --noEmit, pnpm build`
- [ ] 5.2 [P] Update `README.md`: replace all `npm install` / `npm run` references with pnpm equivalents
- [ ] 5.3 [P] Update `CONTRIBUTING.md`: replace npm command references with pnpm; add a note that contributors must run `pnpm changeset` when submitting PRs that need a version bump
- [ ] 5.4 [P] Update `docs/maintenance-policy.md`: replace npm command references with pnpm
- [ ] 5.5 [P] Update `AGENTS.md`: replace npm command references with pnpm
- [ ] 5.6 Commit: `docs: update npm references to pnpm across docs and config`

## 6. Verification

- [ ] 6.1 Run `pnpm test` — confirm all tests pass
- [ ] 6.2 Run `pnpm lint` — confirm no lint errors
- [ ] 6.3 Run `pnpm exec tsc --noEmit` — confirm no type errors
- [ ] 6.4 Run `pnpm build` — confirm production build succeeds
- [ ] 6.5 Run `pnpm build-storybook` — confirm Storybook build succeeds under pnpm

## 7. First Version Bump with changesets

- [ ] 7.1 Run `pnpm changeset` interactively: select patch bump, write summary "Migrate package manager from npm to pnpm and replace semantic-release with changesets"
- [ ] 7.2 Commit the generated `.changeset/*.md` file: `chore: add changeset for pnpm migration`

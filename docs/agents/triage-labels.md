...
are that kind of work, so they carry the same label; which one it is follows from the status
(ADR-0070). The status each label sits in, and who moves it, is the table in `WORKFLOW.md`.

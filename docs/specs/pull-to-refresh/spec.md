...
Define the behavior of the `usePullToRefresh` hook and `PullRefreshIndicator` component in VolleyBro. The hook provides PWA-aware pull-to-refresh gesture handling with exponential damping, threshold-based callback firing, error surfacing, and minimum display duration. The indicator component renders as a flow-layout sibling that pushes content down during gesture and refresh states.
...
<!-- @trace
source: refactor-pull-to-refresh-hook
updated: 2026-04-27
code:
  - src/lib/features/global-slice.ts
  - AGENTS.md
  - src/lib/redux/store.ts
  - src/components/layout/pull-refresh-indicator.tsx
  - CLAUDE.md
...
tests:
  - src/components/layout/__tests__/pull-refresh-indicator.test.tsx
  - src/hooks/__tests__/use-pull-to-refresh.test.ts
  - src/lib/api/__tests__/error-toast.test.ts
-->

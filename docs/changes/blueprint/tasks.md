...
- [x] [P] 9.1 `content/changes/index.mdx` is rewritten to show two sections — "In Progress" (listing `dialog-elevation-system` with its summary "Redefine background color tokens into a three-layer elevation model and unify Dialog layout") and "Archive" (listing all 4 archived changes with name, date, and a brief description) — each archived entry links to its index page at `/changes/archive/<date-name>/`; the in-progress entry has no blueprint page yet so it displays name and summary only — verify by loading `/changes` in dev server and confirming both sections render with correct links
- [x] [P] 9.2 Four `index.mdx` files are created — one per archived change under `blueprint/content/changes/archive/<date-name>/index.mdx` — each shows the change name, the date prefix, a one-sentence summary, and a list of artifact links: Proposal (`./proposal`), Design (`/changes/archive/<date-name>/design`), Tasks (`./tasks`), Review (`./review`), and Specs (linked to the first spec page under `./specs/`) — the Design link is an absolute path because fumadocs does not include design in its page tree — verify by loading each change's index in dev server and confirming all links are present and correct
- [x] 9.3 `meta.json` for each of the 4 archived change directories is updated so that "index" is the first entry in the "pages" array (before "proposal", "tasks", "review", "specs") — fumadocs will then render the index page as the default when navigating to the change's sidebar entry — verify by confirming the fumadocs sidebar shows the change name as a clickable entry that lands on the index page
- [x] 9.4 `pnpm --filter blueprint build` completes without errors and the new index pages appear in the static output — verify that `blueprint/out/changes/archive/<date-name>/index.html` exists for all 4 archived changes and that `blueprint/out/changes/index.html` contains both the "In Progress" and "Archive" sections

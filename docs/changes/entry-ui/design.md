...
- **Mockup-fidelity corrections (post-apply ingest).** Human review found the first apply diverged from the `design.tsx` mockup; these are the reconciled decisions. (a) The drawer is the mockup's **custom bottom-anchored translate-y sheet** (Preview = top edge, ~3.5rem idle peek, `translate-y-0` expanded), not a vaul modal — a modal cannot show a persistent peek. (b) The **progress bar and the shown recording panel share one source of truth**, so tap/swipe moves both the highlight and the panel in sync (the earlier draft-derived-vs-`status.panel` split read as "can't switch"). (c) Completed-or-active segments fill **primary** (mockup style 5), pending muted. (d) The panel body **slides directionally** on step switch (reusing `tailwindcss-animate` rather than the mockup's raw keyframes). (e) The recording panel drops its own per-step titles — the progress-bar caption is the sole label. (f) Submission is **only** the Preview; the away move button's second-tap submit and its send icon are removed.
- **Layout / drawer / animation corrections (second review pass).** A later review on the running UI reconciled the following, revising parts of the bullet above. (a) **Single-viewport layout**: the entry route is one `fixed inset-0` viewport-height column (header height reserved once, then court / panel / peek), replacing the nested-layout double padding that produced an oversized header–court gap and pushed the drawer off-screen. (b) **The expanded drawer is a modal after all** — requirements changed from the mockup: the user wants a backdrop overlay and dialog-scale height, which the panel-bound translate-y sheet could not give. So the idle **peek stays inline** (handle + Preview) while the **expanded** state is a vaul modal (portalled to `<body>`, overlay, up to `85dvh`); this supersedes point (a) of the previous bullet for the expanded state only. (c) **The moves body scrolls within the panel** (`min-h-0` flex chain + `overflow-y-auto`) so a long list never overflows onto the peek. (d) **The slide keys on the derived step (0/1/2), not `status.panel`**, so selecting a player (step 0 → 1, panel unchanged) still animates. (e) **Graceful submit**: `createRally`/`updateRally` re-throw so the optimistic mutate rolls back instead of caching `undefined`; the draft is confirmed only on success; failures surface a toast — the backend `CastError` (an undefined away lineup on that game) is a pre-existing server-side issue, out of scope here.

...
- [ ] 7.2 Remove the redundant submit affordance on the away move buttons so the Preview is the sole submission control. Verify: tapping a selected away move again does not submit and shows no send icon (scenario "Away move button does not submit").
- [ ] 7.3 Remove the recording panel's own per-step titles so the progress-bar caption is the sole step label. Verify: the shown moves panel renders no our-team / opponent titles (scenario "Moves panel shows no redundant step titles").
- [ ] 7.4 Fill every completed-or-active progress segment with the primary color (pending = muted, active wider). Verify: a completed step renders the primary fill (scenario "Completed step shows the primary fill").
- [ ] 7.5 Drive the progress-bar highlight and the shown recording panel from one source of truth so tap/swipe switches both in sync, and animate the panel body with a directional slide on switch. Verify: tapping a reachable segment swaps the panel and highlight together, sliding in by direction (scenarios "Tapping a reachable step swaps the panel in sync", "Panel body slides directionally on switch").

...
import TeamForm, { EditTeamWorkspace, NewTeamWorkspace } from "@/components/team/form";
...
describe("EditTeamWorkspace", () => {
  ...
  it("renders loading skeleton while team data is loading", () => {
    (useTeam as jest.Mock).mockReturnValue({ team: undefined, isLoading: true, error: undefined, mutate: jest.fn() });
    render(<EditTeamWorkspace teamId="team-123" />);
    expect(screen.queryByPlaceholderText("日本國家男子排球隊")).not.toBeInTheDocument();
  });

  it("renders team form when team data is loaded", () => {
    (useTeam as jest.Mock).mockReturnValue({ team: { name: "Test Team", nickname: "TT" }, isLoading: false, error: undefined, mutate: jest.fn() });
    render(<EditTeamWorkspace teamId="team-123" />);
    expect(screen.getByPlaceholderText("日本國家男子排球隊")).toBeInTheDocument();
  });

  it("renders not-found alert with 返回 when team is null and not loading", () => {
    (useTeam as jest.Mock).mockReturnValue({ team: undefined, isLoading: false, error: undefined, mutate: jest.fn() });
    render(<EditTeamWorkspace teamId="team-123" />);

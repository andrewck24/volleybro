...
Optional execution coordination that reuses the repository-owned delivery contract.

## Current behavior

- Symphony polls eligible work, claims a Change, creates an isolated workspace, and invokes the approved Apply contract.
- Symphony dispatches and keeps running only issues that carry `ready-for-agent` in an active status (Todo or In Progress). A run that needs the developer swaps the label for `ready-for-human` and keeps its status, which stops it; re-arming resumes it from In Progress.
